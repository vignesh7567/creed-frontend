...
const services = [
    ...
    },
    {
        ...
        "image": serviceimg4,
        "reDireactLink": "/corporate-uniforms"
    },
    {
        "id": 5,
        ...
        "image": serviceimg5,
        "reDireactLink": "/brand-creative-direction"
    }
]
...
const OurServices = () => {
  ...
  return (
    <div className=' flex flex-col gap-[100px]'>
      <div className=' flex text-white text-center items-center justify-center text-[40px] font-[800] tracking-widest'>OUR SERVICES</div>
      <div className=' flex flex-wrap justify-center items-stretch gap-[20px]'>
        {services.map(service => (
            <div key={service.id} className="bg-white rounded-lg shadow flex flex-col w-[400px] overflow-hidden border-[1px] border-[rgba(255,255,255,0.7)]">
                <img src={service.image} alt={service.title} className="w-full object-contain" />
                <div className="text-[30px] font-[700] px-[20px] leading-7 pt-[13px] pb-[3px]">{service.title}</div>
                <p className="mb-[82px] text-[16px] px-[20px] text-[rgba(20,25,66,0.7)]">{service.description}</p>
                <button onClick={()=>{navigate(service.reDireactLink)}} className="mt-auto mx-[20px] mb-[20px] text-[20px] font-[500] px-4 py-2 bg-[rgba(212,50,61,1)] border-1 border-[rgba(212,50,61,0.45)] hover:bg-white hover:text-red-600 hover:border-red-600 cursor-pointer text-white rounded">
                    EXPLORE SERVICES
                </button>
            </div>
        ))}
